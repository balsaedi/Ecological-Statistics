...
iv. Compare the test statistic to the critical value to assess the plausibility of the null hypothesis. For symmetrical distributions(e.g t, normal), critical values are located on;
...
* We use a one-sample t-test since the population data standard deviation is unknown, and the sample size is small(n=30). The formula for t-statistic is calculated as; $$t = {{\overline{x} - \mu_0}\over{s/\sqrt{n}}}$$
Where: 
...
When modelling the parametric one-way ANOVA, the **F-statistic** is computed as; 
$${F} = {{\text Between-Groups Variance(MSG)}\over{\text Within-Group Variance(MSEE)}}$$
Where;
...
You are required to analyze the forest health and ecological diversity by performing various hypothesis testing. The data set can be downloaded from [here](https://www.kaggle.com/datasets/ziya07/forest-health-and-ecological-diversity) -- use the `forest_health_data_with_target.csv` Use the . Here is the breakdown of some of the data set attributes;
...
The exercise will cover what we have just learnt above. 
...
3. Perform the parametric independent two-sample t-test to compare the mean DBH between 'Healthy' and 'Unhealthy' trees.
